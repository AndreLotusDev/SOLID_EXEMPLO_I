...
namespace EXEMPLO_I_CERTO.Banco
{
    public class GerenciadorBanco<Elemento> where Elemento : EntidadeBase
    {
        ...
        {
            elemento.Id = ObjectId.NewObjectId();
            var colecao = MapeaTabela();
        ...
        public Elemento PegaUm(ObjectId id)
        {
            var colecao = MapeaTabela();

            var elementoEncontrado = colecao.Query().Where(e => e.Id == id).FirstOrDefault();
